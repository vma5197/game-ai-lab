...
def get_health_potion_count(data):
    ...
    for p in data["inventory"]["potions"]:
...
def get_active_quest_titles(data):
    ...
    return [q["title"] for q in data["quests"]["active"]]
...
def calculate_total_weapon_damage(data):
    ...
    return sum(w["damage"] for w in data["inventory"]["weapons"])
